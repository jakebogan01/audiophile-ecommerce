...
<section class="px-6">
     {#each $preferences as product}
          {#if product?.category === category && product?.slug === data?.slug}
               <div class="flex flex-col md:flex-row md:items-center space-y-10 md:space-y-0 md:space-x-[4.3125rem] desktop:space-x-[7.8125rem] max-w-[69.375rem] mx-auto">
                    <div class="flex-1 flex justify-center items-center bg-dark-gray md:max-w-[17.5625rem] md:h-[30rem] rounded-lg overflow-hidden">
                         <img src={product?.image?.desktop} alt={product?.title} class="h-[20.4375rem] md:h-auto mx-auto">
                    </div>

                    <div>
                         {#if product?.new}                                   
                              <h3 class="font-normal text-dark-orange text-sm md:text-xs tracking-[0.625rem]">NEW PRODUCT</h3>
                         {/if}

                         <h1 class="max-w-[18.75rem] desktop:ml-0 font-bold text-[1.75rem] tracking-[0.080625rem] leading-[2.5rem] md:leading-[2rem] my-6 uppercase">{product?.title}</h1>
                         <p class="max-w-[21.8125rem] desktop:ml-0 font-medium text-[0.9375rem] text-[#7D7D7D] leading-[1.5625rem]">{product?.description}</p>
               ...
               <div class="flex flex-col md:flex-row mt-24 space-y-5 md:space-y-0 md:space-x-[1.125rem] max-w-[43.0625rem] mx-auto">
                    <div class="flex-1 flex flex-col justify-between">
                         <div class="galleryOne bg-center bg-no-repeat bg-cover h-[10.875rem] md:h-[10.875rem] w-full rounded-lg overflow-hidden" style="background-image: url({product?.gallery?.first?.desktop});"></div>
                         <div class="galleryOne bg-center bg-no-repeat bg-cover h-[10.875rem] md:h-[10.875rem] w-full rounded-lg overflow-hidden" style="background-image: url({product?.gallery?.second?.desktop});"></div>
                    </div>
                    <div class="galleryOne flex-1 bg-center bg-no-repeat bg-cover h-[23rem] rounded-lg overflow-hidden" style="background-image: url({product?.gallery?.third?.desktop});"></div>
               </div>
          {/if}
     {/each}
</section>